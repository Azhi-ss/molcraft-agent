...
3. **提出改进假设**：
   ...
   Q3 本假设的最低可接受结果是什么？
   答：top-10 平均结合能下降不超过 5%，且 trivial 比例降至 3/10 以下
...
1. **运行实验**（推荐使用 `run_pipeline` 工具，默认开启对接引导）：
   ...
   **⚡ 铁律：H002 Docking Guidance 禁止关闭。** 基线实验已证明 `--docking-guidance` 将结合能从 -7.7~-8.1 提升至 -8.56 kcal/mol（+0.4~0.8 kcal/mol）。任何实验中关闭它来"隔离变量"都是无效做法——因为关闭后基线本身就退化 0.5 kcal/mol，无法判断新改动的净效应。正确的对照方法：始终开 docking guidance，只在其他变量上做 A/B。
